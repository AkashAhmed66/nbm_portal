<x-app-layout>
    <div class="max-w-6xl mx-auto mt-8">
        ...
        <div class="flex justify-end mt-10">
            <a href="{{ route('projects.create') }}" class="px-2 py-1 rounded-md bg-blue-500 text-sky-100 hover:bg-blue-700">+ Create New</a>
        </div>
                ...
                <div class="inline-block min-w-full overflow-hidden align-middle border-b border-gray-200 shadow sm:rounded-lg">
                    ...
                    <table class="min-w-full">
                        <tr>
                            ...
                            <th class="px-6 py-3 text-xs font-medium leading-4 tracking-wider text-left text-gray-500 uppercase border-b border-gray-200 bg-gray-50">SN</th>
                        ...
                        @foreach ($data as $item)
                            <tr>
                                ...
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                    <img class="h-12 w-12 object-cover rounded" src="{{ asset($item->image) }}" alt="{{ $item->title }}">
                                </td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ $item->serviceCategory->serviceType->title }}</td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ $item->serviceCategory->title }}</td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ $item->title }}</td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ $item->clientName }}</td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ $item->location }}</td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">{{ $item->sln }}</td>
                                <td class="px-6 py-4 whitespace-no-wrap border-b border-gray-200">
                                    <form action="{{ route('projects.destroy',$item->id) }}" method="POST">

                                        <a href="{{ route('projects.edit',$item->id) }}" class="text-indigo-600 hover:text-indigo-900 text-gray-600">
